...
let show_body_atom a =
  ...
  if !use_xml then 
  "<pred>"^(if a.marked then "K+" else if a.recipize then "K-" else "K")^"<loc>"^(show_loc_set a.loc)^"</loc>(<rec>"^(show_term a.recipe)^"</rec>,<term>"^(show_term a.term)^"</term>)</pred>"
  else
   (if a.marked then "KnOwS_" else if a.recipize then "recip_" else "knows_")^(show_loc_set a.loc)^"("^(show_term a.recipe)^","^(show_term a.term)^")"
